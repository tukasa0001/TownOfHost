...
| マッドメイト系役職がコミュサボを直すことができる |
| マッドメイト系役職の視界がインポスターと同じ |
| マッドガーディアンが自身をキルしたインポスターを見ることができる |
| マッドガーディアンのタスク数 |
...
有効化することで、ロビーのコードを非表示にすることができます。

コンフィグファイル(BepInEx\config\com.emptybottle.townofhost.cfg)の``Hide Game Code Name``を書き換えることで、HideGameCodesを有効化したときの文字を変えることができます。
また、``Hide Game Code Color``を書き換えることによって文字の色も好きなように変更できます。

### JapaneseRoleName

有効化することで、役職名を日本語で表示させることができます。
クライアントの言語を英語にしている場合、ホストが``日本語に強制``を有効化していないと、この設定は適用されません。
