...
| [ヴァンパイア](#Vampireヴァンパイア)                                 | [スニッチ](#Snitchスニッチ)                                 | [シーフ](#thiefシーフ)                                      |           |  |
...
初期状態では勝利条件を持ちません。<br>
キル可能役職(各種インポスター、[エゴイスト](#egoistエゴイスト)、[ジャッカル](#jackalジャッカル)、[シェリフ](#sheriffシェリフ))のプレイヤーにキルを行うことで、相手を殺害し、役職を奪うことができます。<br>
キル可能役職以外に対してキルを行おうとした場合、自爆してしまいます。<br>
「盗まれたプレイヤーの役職を変更する」設定をオンにすると、役職を奪われたプレイヤーは[オポチュニスト](#opportunistオポチュニスト)として表示されるようになり、勝利することが不可能になります。<br>
同設定がオフの場合、元の役職での勝利条件が満たされると盗まれたプレイヤーも一緒に勝利します。<br>

注意点
* インポスターやエゴイストを盗んだ場合、他のインポスターにもキルボタンが反応しますが、押しても何も起こりません。
* 「盗まれたプレイヤーの役職を変更する」設定がオンの状態でインポスター役職を盗んだ場合、盗まれたプレイヤーはサボタージュができなくなります。仕様上ドア閉鎖は可能ですが、行わないでください。
* 仕様上、役職を盗む前、またはシェイプシフター判定ではない役職を盗んだ非Modプレイヤーには、クールダウン255秒の変身ボタンが表示されます。
...
[バウンティーハンター](#BountyHunter/バンティーハンター)や[マフィア](#Mafia/マフィア)、[ヴァンパイア](#Vampire/ヴァンパイア)、[魔女](#Witch/魔女)、[ベイト](#Bait/ベイト)、[メイヤー](#Mayor/メイヤー)、[シェリフ](#Sheriff/シェリフ)、[スニッチ](#Snitch/スニッチ)、[ライター](#Lighter/ライター)、[シーア](#Seer/シーア)、[ジャッカル](#jackalジャッカル)、[シーフ](#thiefシーフ) のアイデア元であり、 Mod の作成方法の参考元 : [The Other Roles](https://github.com/Eisbison/TheOtherRoles)<br>
