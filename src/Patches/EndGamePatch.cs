...
class EndGamePatch
{
    ...
    public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)] ref EndGameResult endGameResult)
    {
        ...
        foreach (var kvp in TOHPlugin.PlayerStates.OrderBy(x => x.Value.RealKiller.Item1.Ticks))
        {
            ...
            KillLog += $"\n{date:T} {TOHPlugin.AllPlayerNames[targetId]}({Utils.GetDisplayRoleName(targetId)}{Utils.GetSubRolesText(targetId)}) [{Utils.GetVitalText(kvp.Key)}]";
...
class SetEverythingUpPatch
{
    ...
    public static void Postfix(EndGameManager __instance)
    {
        ...
        if (AmongUsClient.Instance.AmHost && TOHPlugin.PlayerStates[0].MainRole == CustomRoles.GM)
        {
            ...
            __instance.WinText.color = Utils.GetRoleColor(CustomRoleManager.Special.GM);
            __instance.BackgroundBar.material.color = Utils.GetRoleColor(CustomRoleManager.Special.GM);
        }
