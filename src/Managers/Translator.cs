...
public static class Translator
{
    ...
    {
        VentLogger.Old("Language Dictionary Initialize...", "Translator");
        LoadLangs();
        VentLogger.Old("Language Dictionary Initialize Finished", "Translator");
    }
    public static void LoadLangs()
    {
        var assembly = System.Reflection.Assembly.GetExecutingAssembly();
        var stream = assembly.GetManifestResourceStream("TownOfHost.assets.string.csv");
        ...
        foreach (var line in CsvReader.ReadFromStream(stream, options))
        {
            ...
            try
            {
                ...
                if (!translateMaps.TryAdd(line.Values[0], dic))
                    VentLogger.Warn($"翻訳用CSVに重複があります。{line.Index}行目: \"{line.Values[0]}\"", "Translator");
            }
            catch (Exception ex)
            {
                VentLogger.Warn(ex.ToString(), "Translator");
            }
        }
    ...
    public static void LoadCustomTranslation(string filename, SupportedLangs lang)
    {
        ...
        if (File.Exists(path))
        {
            VentLogger.Old($"カスタム翻訳ファイル「{filename}」を読み込み", "LoadCustomTranslation");
            ...
            while ((text = sr.ReadLine()) != null)
            {
                ...
                if (tmp.Length > 1 && tmp[1] != "")
                {
                    ...
                    {
                        VentLogger.Warn($"「{tmp[0]}」は有効なキーではありません。", "LoadCustomTranslation");
                    }
                }
            }
        }
        ...
        {
            VentLogger.Error($"カスタム翻訳ファイル「{filename}」が見つかりませんでした", "LoadCustomTranslation");
        }
    }
