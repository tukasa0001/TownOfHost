...
namespace TownOfHost
{
    ...
    class PageObject {
        ...
        public PageObject( //サブ役職設定
            ...
            this.parent = parent; //親オブジェクト
            this.getName = () => $"<color={main.getSubRoleColorCode(subRole)}>{main.getRoleName(subRole)}</color>: {main.GetCountFromSubRole(subRole)}";
            this.isHostOnly = false; //実行をホストのみに限定するか
            this.onEnter = () => main.SetSubRoleCountToggle(subRole); //実行時の動作
