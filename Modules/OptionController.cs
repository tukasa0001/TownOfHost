...
namespace TownOfHost
{
    static class CustomOptionController
    {
        ...
        public static void begin() {
            ...
            var EnableLastImpostor = new PageObject(AdvImpostorRoleOptions, () => $"<color={Utils.getRoleColorCode(CustomRoles.Impostor)}>{getString("EnableLastImpostor")}</color>: {Utils.getOnOff(Options.EnableLastImpostor)}", true, () => {Options.EnableLastImpostor = !Options.EnableLastImpostor;});
            var LastImpostorKillCooldownDiscount = new PageObject(AdvImpostorRoleOptions, () => $"<color={Utils.getRoleColorCode(CustomRoles.Impostor)}>{getString("LastImpostorKillCooldownDiscount")}</color>(s): {Options.LastImpostorKillCooldownDiscount}{main.TextCursor}", true, () => {Options.LastImpostorKillCooldownDiscount = 0;}, (n) => Utils.ChangeInt(ref Options.LastImpostorKillCooldownDiscount, n, 180));
