        ...
        new LateTask(() =>
        {
            switch (Reason)
            {
                ...
                case DisconnectReasons.Error:
                    //if (StringReason.Contains("Couldn't find self")) __instance.SetText(GetString("DCNotify.BadNet"));
                    if (StringReason.Contains("Failed to send message")) __instance.SetText(GetString("DCNotify.BadNet"));
                    break;
                case DisconnectReasons.Custom:
                    if (StringReason.Contains("Reliable packet")) __instance.SetText(GetString("DCNotify.BadNet"));
                    else if (StringReason.Contains("remote has not responded to")) __instance.SetText(GetString("DCNotify.BadNet"));
                    break;
            }
