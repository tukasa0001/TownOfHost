    ...
    class SelectRolesPatch
    {
        public static void Prefix()
        {
            ...
            if (Options.CurrentGameMode != CustomGameMode.HideAndSeek)
            {
                ...
                int AdditionalShapeshifterNum = CustomRoles.SerialKiller.GetCount()
                                            + CustomRoles.BountyHunter.GetCount()
                                            + CustomRoles.Warlock.GetCount()
                                            /* + CustomRoles.ShapeMaster.GetCount()*/
                                            + CustomRoles.FireWorks.GetCount()
                                            + CustomRoles.Sniper.GetCount()
                                            + CustomRoles.EvilTracker.GetCount()
                                            + CustomRoles.ShapeKiller.GetCount();//- ShapeshifterNum;
        ...
        public static void Postfix()
        {
            ...
            else
            {
                ...
                int ShapeshifterNum = roleOpt.GetNumPerGame(RoleTypes.Shapeshifter);
                ShapeshifterNum -= CustomRoles.SerialKiller.GetCount()
                                + CustomRoles.BountyHunter.GetCount()
                                + CustomRoles.Warlock.GetCount()
                                /* + CustomRoles.ShapeMaster.GetCount()*/
                                + CustomRoles.FireWorks.GetCount()
                                + CustomRoles.Sniper.GetCount()
                                + CustomRoles.EvilTracker.GetCount()
                                + CustomRoles.ShapeKiller.GetCount();
                if (Main.RealOptionsData.NumImpostors > 1)
